<template>
  <div class="bg0" style="position: absolute;top: 0;left: 0;right: 0;bottom: 0;">
    ...
    <div class="content-box">
      ...
      <div class="register-down-app">
        <div style="border-bottom: solid 1px #ff293c;" @click="downloadApp">下载APP</div>
      </div>
    </div>
      ...
      listenerConfirmClick(dt){
        ...
        this.$http.post((`${window.Host.apiHost}/user/register`), data).then((res) => {
          ...
          if (res.data.stateCode === 0) {
            ...
            var toastInstanse = Toast({message: '注册成功，去下载APP吧', position: 'center'});
            setTimeout(() => {
              toastInstanse.close();
              this.$router.push({path: `/downAPP`})
            },3000);
