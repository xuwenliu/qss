<template>
  <div class="bg0" style="position: absolute;top: 0;left: 0;right: 0;bottom: 0;">
    ...
    <TopHeader>
      ...
      <i>{{type==1?'注册新账号':'找回密码'}}</i></TopHeader>
    ...
    <div class="content-box">
      ...
      <div class="reg-user-argument" v-show="type == 1">
      ...
      <div class="login-button" style="margin-top: 0.66666rem">
        <button class="fc0 f28" :class="{'login-button-n':loginBtnEnable}"
                @click="listenerConfirmClick([bindUsernameInput,bindCodeInput,bindPwasswordInput])">{{type==1?'注册':'确认'}}
      </button>
      </div>
    </div>
  </div>
  ...
  export default{
    ...
    goBack(){
      ...
      listenerSendSmsCodeClick(dt){
        ...
        this.$http.post(`${window.Host.apiHost}/user/sendSmsCode?phone=${dt}&source=${this.type == 2 ? 2 : 1}`).then((res) => {
          console.log(res);
      ...
      listenerConfirmClick(dt){
        ...
        this.$http.post(((this.type == 1) ? `${window.Host.apiHost}/user/register` : `${window.Host.apiHost}/user/getBackPassword`), data).then((res) => {
