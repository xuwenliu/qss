  ...
  <div class="dealRecord">
    ...
    <div class="content content-box">
          ...
          <div v-for="(item,index) in logs" :class="{'bb1':index!=logs.length-1}"
            ...
            <div class="item-item-main">
              ...
              <div class="item-center">
                <span style="margin-top:0.10666rem; text-algin:center; word-break: break-all" class="f26 fc2">{{item.bankName+item.cardType}}（{{item.cardNO | last4}}）</span>
                <span class="f26 fc1">{{item.transactionDate | setTime}}</span>
              </div>

              <div class="item-right">
                <span v-show="!item.arrived" class="f24" style="color: #b2b2b2;margin-right: 0.2rem">处理中</span>
      ...
      <NoFile v-else>
        ...
        <span slot="info" class="f32" style="color: #282828">您还没有交易记录</span>
      </NoFile>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      /*默认数据*/
      getDataInfo(){
        this.$http.get(`${window.Host.apiHost}/pos/records?pageNum=${this.pageNum}&pageSize=${this.pageSize}`).then( (res)=> {
          console.log(res);
